...
public class BuildinAddAndReturnGeneratedIdBuilderFactory extends AbstractBuildinBuilderFactory {
  ...
  AbstractBuildinBuilder createInternalBuilder(CrudMeta cm) {
    String property4AutoGeneratedValue = cm.getProperty4AutoGeneratedValue();
    if (property4AutoGeneratedValue == null) { // 没有使用 @AutoGeneratedValue 注解，却调用生成自增主键的方法
        throw new IllegalStateException("No @AutoGeneratedValue, but called addAndReturnGeneratedId");
    }
    return new BuildinAddBuilder(property4AutoGeneratedValue, cm.getProperties(), cm.getColumns());
  }
