...
public class ShardStrategyAndTypeTest {
    ...
    interface ProductDao {
        ...
        int insert(@ShardBy(value = "id", type = TYPE_ID) Product product);

        @SQL("select id, uid, price from #table where id = :1")
        public Product getProductById(@ShardBy(type = TYPE_ID) String id);

        @SQL("select id, uid, price from #table where uid = :1")
        public Product getProductByUid(@ShardBy(type = TYPE_UID) String uid);
    ...
    static class ProductShardStrategy implements ShardStrategy<String> {
